...
const signInWithGoogle = async (setUser) => {
  ...
  try {
    ...
    alert('user accessToken ' + JSON.stringify(user.auth.accessToken))
    alert('333')
    if (type === 'success') {
      await GoogleSignIn.initAsync({
        clientId: '404097902962-quufj5t480gmgr9e9r0brvovankauun0.apps.googleusercontent.com',
      })
      const users = await GoogleSignIn.signInSilentlyAsync()
      alert('users accessToken ' + JSON.stringify(users.auth.accessToken))
      setUser(users)
